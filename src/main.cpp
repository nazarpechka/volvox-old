...
const string strMessageMagic = "VOLVOX Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in VOLVOX
    bool fStrictPayToScriptHash = true; // Always active in VOLVOX
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    printf("SetBestChain: new best=%s  height=%d  trust=%s  tx=%lu date=%s progress=%f%\n",
      hashBestChain.ToString().substr(0,20).c_str(), nBestHeight, bnBestChainTrust.ToString().c_str(), (unsigned long)pindexNew->nChainTx,
...
//vMerkleTree: ea6fed5e2
        // Genesis block
        const char* pszTimestamp = "March 30, 2018, The Times, Hundreds gather for funeral of Stephen Hawking. RIP";
				if(fTestNet)
					pszTimestamp = "Volvox Testnet";
        CTransaction txNew;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hashPoW > hashTarget && pblock->IsProofOfWork())
        return error("VOLVOXMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VOLVOXMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("VOLVOXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadVOLVOXMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void VOLVOXMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("VOLVOX-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running VOLVOXMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadVOLVOXMiner(void* parg)
{
    ...
    try
    {
        ...
        VOLVOXMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadVOLVOXMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadVOLVOXMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadVOLVOXMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d VOLVOXMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadVOLVOXMiner, pwallet))
                printf("Error: NewThread(ThreadVOLVOXMiner) failed\n");
            MilliSleep(10);
        }
    }
}
